 ...
 * @since 2.0
 */
class MyMigrateController extends BaseMigrateController
{
    /**
     ...
     * @since 2.0.8
     */
    protected function generateMigrationSourceCode($params, $prefix = 'quiz_')
    {
        ...
        $table = null;
        if (preg_match('/^create_junction_(.+)_and_(.+)_for_(.+)_and_(.+)_tables?$/', $name, $matches)) {
            $templateFile = $this->generatorTemplateFiles['create_junction'];
            $firstField = $matches[1];
            $secondField = $matches[2];
            $firstTable = $matches[3];
            $secondTable = $matches[4];

            $fields = array_merge(
                [
                    [
                        'property' => $firstField . '_id',
                        'decorators' => 'integer()',
                    ],
                    [
                        'property' => $secondField . '_id',
                        'decorators' => 'integer()',
                    ],
                ],
                $fields,
            ...
            $foreignKeys[$firstField . '_id']['table'] = $firstTable;
            $foreignKeys[$secondField . '_id']['table'] = $secondTable;
            $foreignKeys[$firstField . '_id']['column'] = null;
            $foreignKeys[$secondField . '_id']['column'] = null;
            $table = $firstField . '_to_' . str_replace($prefix, '', $secondField);
